...
if (pinCodeAnswer.pincode === pincode) {
    console.log(chalk.italic.bold.greenBright.bold("\n\tcorrect your pin code.\n"));
...
let operationAnswer = await inquirer.prompt([
    {
        ...
        type: "list",
        choices: ["withdraw", "check balance", "fast cash", "funds transfer", "change pin", "exit"],
        transformer: (input) => {
            return chalk.italic.blue.bold(input.toString());
        }
    }
...
if (operationAnswer.operations === "check balance") {
    ...
    const newPinAnswer = await inquirer.prompt({
        ...
        transformer: (input) => chalk.italic.blue.bold(input.toString()),
    });
